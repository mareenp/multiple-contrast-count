...
## Type-I Error
TI3 <- read_excel("MCT_Count_typeI_3groups.xlsx")
## Power
P3 <- read_excel("MCT_Count_power_3groups.xlsx")

## change directory to the folder for plots
setwd("../Results_Plots/")

############################# Type-I Error ####################################
TI3_long <- TI3 %>%  rename(Type_I_Error = global) %>%
...
labels <- c("(1) POI balanced homoscedastic", 
            ...
            expression(paste("(3) CMP balanced homoscedastic ", italic(nu)[1],"=",italic(nu)[2],"=",italic(nu)[3],"=0.5")),
            expression(paste("(4) CMP balanced homoscedastic ", italic(nu)[1],"=",italic(nu)[2],"=",italic(nu)[3],"=2")), 
            expression(paste("(5) CMP balanced heterosc. ", italic(nu)[1],"=0.2, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=0.5")), 
            expression(paste("(6) CMP balanced heteroscedastic ", italic(nu)[1],"=0.2, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=2")),
            expression(paste("(7) CMP positive pairing ", italic(nu)[1],"=0.5, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=0.2")),
            expression(paste("(8) CMP positive pairing ", italic(nu)[1],"=2, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=0.2")),
            expression(paste("(9) CMP negative pairing ", italic(nu)[1],"=0.2, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=0.5")),
            expression(paste("(10) CMP negative pairing ", italic(nu)[1],"=0.2, ", italic(nu)[2],"=0.5, ", italic(nu)[3],"=2")),
            expression(paste("(11) NB balanced homoscedastic ", size[1],"=",size[2],"=",size[3],"=3")), 
            expression(paste("(12) NB positive pairing ", size[1],"=5, ", size[2],"=3, ", size[3],"=2")),
            expression(paste("(13) NB negative pairing ", size[1],"=0.75, ", size[2],"=2, ", size[3],"=3")),
            expression(paste("(14) NB negative pairing ", size[1],"=2, ", size[2],"=3, ", size[3],"=5")))

plots <- map(as.list(unique(TI3_long$no)),
             function(b) {
               TI3_long %>% filter(no == b) %>% 
...
############################ Power #############################################
################################################################################
P3_long <- P3 %>%  rename(Power = global) %>%
...
plots <- map(as.list(unique(P3_long$no)),
             function(b) {
               P3_long %>% filter(no == b) %>% 
...
png(filename = "PlotPower_POI_3.png",
    width = 3000, height = 1600, res = 300)
(p1 | p2 ) / 
  leg + plot_layout(heights = c(1,0.1))
dev.off()
...
png(filename = "PlotPower_CMP1_3.png",
    width = 3000, height = 3200, res = 300)
(p1 | p3 ) / 
  (p2 | p4) /
  leg + plot_layout(heights = c(1,1,0.1))
...
png(filename = "PlotPower_CMP2_3.png",
    width = 3000, height = 3200, res = 300)
(p1 | p3 ) /
  (p2 | p4) /
  leg + plot_layout(heights = c(1,1,0.1))
...
png(filename = "PlotPower_NB_3.png",
    width = 3000, height = 3200, res = 300)
(p1 | p3 ) / 
  (p2 | p4) /
  leg + plot_layout(heights = c(1,1,0.1))
